...
namespace Utrecht {
	...
	class Utrecht {
		...
		static void ProcessInterface (WlanInterface WlanInterface) {
			...
			while (true) {
				Writer.WriteLine();
				Writer.Write("조사할 SSID의 번호를 입력하세요.\n[q: 종료, a: 모든 SSID 탐색, s: SSID 목록 표시, o: 설정 변경]: ");
				var InputString = Console.ReadLine();
				if (InputString == "q") {
					return;
				} else if (InputString == "a") {
					foreach (var network in NetworkList) {
						ProcessNetwork(WlanInterface, network);
					}
					...
					switch(InputString) {
						case "1":
							Writer.Write("변경할 주소를 입력하세요. [현재: {0}]: ", FTPServerHostname);
							InputString = Console.ReadLine();
						...
						case "2":
							...
							break;
						case "3":
							Writer.Write("변경할 주소를 입력하세요. [현재: {0}]: ", ServerHostname);
							InputString = Console.ReadLine();
		...
		static void ProcessNetwork (WlanInterface WlanInterface, Wlan.WlanAvailableNetwork Network) {
			string profileName = Network.profileName;
			string profileXML = WlanInterface.GetProfileXml(profileName);
				...
				int BSSIDIndex = 0;
					...
					case "1": case "2": case "3":
						foreach (var BSSID in BSSIDList) {
							...
							WlanInterface.ConnectSynchronously(NativeWifi.Wlan.WlanConnectionMode.Profile, NativeWifi.Wlan.Dot11BssType.Infrastructure, profileName, 5000, MacAddress);
		...
		static string FTPServerHostname = "ftp://elenesgu.asuscomm.com/test/100MB.txt";
		...
		static Tuple<PingResult, PingResult> MeasureNetworkStatus (Wlan.WlanAvailableNetwork Network) {
			try {
				...
				return Tuple.Create(trace[0] ,trace[trace.Count - 1]);
			}
			catch (PingException) {
				Console.WriteLine("Ping 전달 중 이상이 발생했습니다.");
				return null;
			}
		}
